...
| Spawn inputs               | Child inherits `projectId`, `worktreePath`, `branch`, `runtimeMode`, `interactionMode` from the parent. Caller sets `prompt`, `title`, `providerInstanceId`, `model`, `allowOrchestration`, `effort`. Effort resolves like a native spawn: explicit (must be a level the target model's catalog offers, else `invalid_input`) → the parent's effective effort when the target offers it → the target's default; stored under the target driver's option id (`effort` / `reasoningEffort`) and on every row. No per-spawn permission or worktree override. |
...
- `spawn({ providerInstanceId, model, prompt, title?, allowOrchestration })`:
  ...
  4. Dispatch `thread.create` with `spawn` metadata, then `thread.turn.start` with the prompt. Emit `task.started` on the parent via the internal activity command with the linkage bundle (`title`, `role: "cross-provider"`, `model`, `effort`, `taskType: "cross_provider_agent"`, `timelineBypass: true`, `childThreadId` in payload) so the row appears in Direct Spawns immediately. The settlement watcher then mirrors the child's own `context-window.updated` (cumulative `totalProcessedTokens` only; context occupancy is never a total) and `tool.started` activities (an exact count, seeded from a COUNT query) onto the row as native-shaped `task.progress` (`typedUsage` + `usageSnapshot`, `lastToolName`) and `tool.progress` rows with ingestion's stable activity ids, coalesced to one mirror per second per child; `task.completed` carries the final cumulative `typedUsage`. Children are excluded from every thread-list surface client-side (one predicate in client-runtime) and from server-side thread search; shell snapshots stay complete so the child remains openable by id.
...
| `agent_spawn`     | `{ providerInstanceId, model, prompt, title?, allowOrchestration?, effort? }` | `{ childId, taskId, providerInstanceId, model, status }`                                                                       |
