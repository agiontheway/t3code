...
function CrossProviderAgentAccessSetting() {
  ...
  useEffect(() => {
    ...
    const currentEntries = Object.entries(aliases);
    const mergedEntries = Object.entries(merged);
    if (
      currentEntries.length === mergedEntries.length &&
      currentEntries.every(([alias, target]) => merged[alias] === target)
    ) {
      return;
    }
  ...
  const updateAliasTarget = (alias: string, target: string | null) => {
    if (!target) return;
    updateSettings({
      crossProviderAgentAliases: {
        ...aliases,
        [alias]: ProviderInstanceId.make(target),
      },
    });
  };

  const restoreAliases = () => updateSettings({ crossProviderAgentAliases: defaultAliases });
            ...
            <div className="space-y-3 border-t border-border/50 px-3 py-3 sm:px-4">
              <div className="flex items-start justify-between gap-4">
                <p className="max-w-2xl text-xs leading-relaxed text-muted-foreground">
                  Friendly names agents may pass to agent_spawn. Each target is an exact provider
                  instance; direct instance IDs still take precedence.
                </p>
                <Button type="button" variant="outline" size="sm" onClick={restoreAliases}>
                  Restore defaults
                </Button>
              </div>
                ...
                <div className="overflow-hidden rounded-lg border border-border/60">
                  {Object.entries(aliases).map(([alias, target], index) => (
                    ...
                    >
                      ...
                      <Select
                        value={target}
                        onValueChange={(value) => updateAliasTarget(alias, value)}
                      >
                        <SelectTrigger
                          size="sm"
